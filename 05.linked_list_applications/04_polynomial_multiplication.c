...
void polynomial_multiplication(struct Node *head1, struct Node *head2)
{
	...
	while (aux1 != NULL)
	{
		while (aux2 != NULL)
		{
			float coeff_product = aux1->coefficient * aux2->coefficient;
			int expo_sum = aux1->exponent + aux2->exponent;
			head3 = insert(head3, coeff_product, expo_sum);
			aux2 = aux2->next;
		}
		aux1 = aux1->next;
		aux2 = head2;
...
int main()
{
	...
	polynomial_multiplication(head1, head2);
	return 0;
}
